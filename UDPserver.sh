...
idioam_lang(){
  source <(curl -sSl 'https://raw.githubusercontent.com/Sonagtech/SocksIP-udpServer/main/lang/lang')
  ...
  for arch in $listarq; do
    if ! wget -O $lang_dir/${list_lang[$lg]}/$arch "https://raw.githubusercontent.com/Sonagtech/SocksIP-udpServer/main/lang/${list_lang[$lg]}/$arch" &>/dev/null ;then
...
repo_install(){
  link="https://raw.githubusercontent.com/Sonagtech/ADMRufu/main/Repositorios/$VERSION_ID.list"
  case $VERSION_ID in
...
if [[ ! -e $udp_file/UDPserver.sh ]]; then
  ...
  check_sistem
	...
	wget -O $udp_file/limitador.sh "https://raw.githubusercontent.com/Sonagtech/SocksIP-udpServer/main/limitador.sh" &>/dev/null
...
[Unit]
Description=UDPserver Service by @sonagtech
...
menu_udp(){
	title "${a1:-SCRIPT DE CONFIGRACION UDPserver} BY @sonagtech"
